...
exports.getPostList = async(userId, cb) => {
    sql.query(`select * from post where sender_id = '${userId}' and is_deleted = 0`, (error, result) => {
...
exports.notifications = async(userId, limit, cb) => {
    sql.query(`SELECT c.user_id, c.post_id,c.created_at, u.user_id, u.first_name, u.last_name, u.user_image, p.sender_id, p.image_url as post_image from comment c left join users u on u.user_id = c.user_id left join post p on p.post_id=c.post_id where p.sender_id = '${userId}' and c.user_id not in (${userId}) limit ${limit}`, (error, notificationResult) => {
        if (error) return cb(error);
